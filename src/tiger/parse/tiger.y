...
%token 
  ...
  BREAK NIL
  FUNCTION VAR TYPE
// expseq: (1; 2; 3) has value 
// nonemptyactuals: 1, 2, 3
// actuals: , 2, 3
// sequencing: (1; 2; 3)
// sequencing_exps 1; 2; 3
// rec: , 2, 3
%type <exp> exp expseq
...
exp: 		INT {$$ = new A::IntExp(errormsg.tokPos, $1);}
  ...
  |     ID LPAREN RPAREN {$$ = new A::CallExp(errormsg.tokPos, $1, NULL);}
  ...
  |     ID LBRACE RBRACE {$$ = new A::RecordExp(errormsg.tokPos, $1, NULL);}
...
sequencing_exps:     {$$ = NULL;}
  |     SEMICOLON exp sequencing_exps {$$ = new A::ExpList($2, $3);};
rec_one:  ID EQ exp {$$ = new A::EField($1, $3);};
rec:                {$$ = NULL;}
  |     COMMA rec_one rec   {$$ = new A::EFieldList($2, $3);};
...
tyfields_nonempty:    tyfield COMMA tyfields {$$ = new A::FieldList($1, $3);}
  |                   tyfield  {$$ = new A::FieldList($1, NULL);};
tyfield:  ID COLON ID {$$ = new A::Field(errormsg.tokPos, $1, $3);};
tyfields:   {$$ = NULL;}
  |         tyfields_nonempty {$$ = $1;};
fundec:   fundec_one    {$$ = new A::FunDecList($1, NULL);}
  |       fundec_one fundec   {$$ = new A::FunDecList($1, $2);};

fundec_one: FUNCTION ID LPAREN tyfields RPAREN EQ exp    {$$ = new A::FunDec(errormsg.tokPos, $2, $4, NULL, $7);}
